...
local function get_proxy()
  return _G.NVIQ.settings.general.proxy or os.getenv("HTTP_PROXY")
end
...
local function download(source_url, download_path, proxy)
  ...
  local download_proc = futures.Process.new(CURL_CMD, { args = args })
...
function M.nvim_upgrade(channel)
  ...
  local upgrader = rsmod.get_bin_path("nviq-upgrade")

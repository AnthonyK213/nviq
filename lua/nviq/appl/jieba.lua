...
function M:init()
  ...
  ffi.cdef [[
void *nviq_jieba_new();
int nviq_jieba_pos(void *jieba, const char *line, int pos, int *start, int *end);
void nviq_jieba_drop(void *jieba);
]]
  _nviq_jieba = ffi.load(dylib_path)
  return true
end
...
---Returns the start and end position of the word at `position`.
---@param sentence string The sentence.
---@param position integer Char index in `sentence` (0-based).
---@return integer? start_pos Start position (0-based, inclusive).
---@return integer? end_pos End position (0-based, inclusive).
function M.get_pos(sentence, position)
  local s, e = ffi.new("int[1]", 0), ffi.new("int[1]", 0)
  if M.jieba
      and _nviq_jieba.nviq_jieba_pos(M.jieba, sentence, position, s, e) == 0 then
    return s[0], e[0] - 1
  end
end

---Returns the word and its position at the cursor.
...
function M.get_word()
  ...
  local word = sutil.sub(line, s_char + 1, e_char + 1)
...
local function prev_char_pos(lnum, pos_byte, line)
  if pos_byte == 0 then
    ...
    pos_byte = #line - #sutil.sub(line, -1, -1)
...
local function next_non_space_char_pos(lnum, pos_byte, line)
  ...
  while true do
    if pos_byte < #line then
      local s_, e_ = _s_spc_pat:match_line(0, lnum - 1, pos_byte)
      if not s_ or not e_ then
        return lnum, pos_byte, line
      end
      pos_byte = pos_byte + (e_ - s_)
...
-- Backup for NVIQ.handlers.get_word
local _get_word_bak = nil
...
function M:enable()
  ...
  if not self.jieba then
    self.jieba = _nviq_jieba.nviq_jieba_new()
    ffi.gc(self.jieba, _nviq_jieba.nviq_jieba_drop)
  end
...
function M:disable()
  if self.enabled then
    NVIQ.handlers.get_word = _get_word_bak
